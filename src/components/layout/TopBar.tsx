    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative group">
          <Search className="w-4 h-4 text-white/20 absolute left-3 top-1/2 -translate-y-1/2 group-focus-within:text-electric-blue transition-colors" />
          <input
            type="text"
            placeholder="Search game & categories"
            className="w-64 bg-white/[0.04] border border-white/[0.06] rounded-xl py-2 pl-9 pr-4 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-electric-blue/30 focus:bg-white/[0.06] focus:w-80 transition-all duration-300"
          />
        </div>

        {/* Notification */}
        <button className="relative w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center text-white/40 hover:text-white/70 hover:bg-white/[0.08] transition-all">
          <Bell className="w-4 h-4" />
          <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-electric-blue rounded-full shadow-[0_0_8px_rgba(0,240,255,0.6)]" />
        </button>

        {/* User - Toggles Admin */}
        <button
          onClick={() => onNavigate(currentView === "admin" ? "home" : "admin")}
          className="flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-xl bg-white/[0.04] border border-white/[0.06] hover:bg-white/[0.08] transition-all group"
        >
