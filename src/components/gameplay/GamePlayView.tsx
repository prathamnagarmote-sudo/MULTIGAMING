      ...
      <div ref={containerRef} className="w-full flex flex-col mb-8">
        ...
        >
            ...
            {isFullscreen && isMobileDevice && (
              <div className="absolute top-0 left-0 right-0 h-[15px] bg-black border-b border-white/[0.05] flex items-center justify-between z-50 select-none">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleFullscreen();
                  }}
                  className="flex items-center gap-1 h-full px-2.5 rounded-r bg-[#7c3aed] hover:bg-[#6d28d9] active:scale-95 text-white font-sans font-bold text-[8px] uppercase tracking-wider transition-all cursor-pointer border-none"
                >
                  <LogOut className="w-[9px] h-[9px]" style={{ transform: "scaleX(-1)" }} />
                  <span>Exit</span>
                </button>
                <span className="text-[7px] font-heading font-black text-white/30 uppercase tracking-widest leading-none pr-3 flex items-center h-full">
                  {game.title}
                </span>
              </div>
            )}
                ...
                isFullscreen
                  ? isMobileDevice
                    ? isPortraitMode
                      ...
                      ? "absolute top-[15px] bottom-0 left-0 right-0 w-full bg-black"
